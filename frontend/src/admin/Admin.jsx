...
export default function Admin() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!urlList.length) {
      setMessage("Please enter at least one YouTube or TikTok URL.");
    ...
    setMessage(
      `Processing ${urlList.length} video${
        urlList.length === 1 ? "" : "s"
    ...
    for (const url of urlList) {
      try {
        ...
        setMessage(`Processed ${added + failed} of ${urlList.length}…`);
    ...
    if (failed === 0) {
      setMessage(`${added} video${added === 1 ? "" : "s"} added and analyzed.`);
    } else {
      setMessage(
      ...
      <section className="card">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="primary" disabled={adding}>
            {adding ? "Adding videos…" : "Add videos"}
          </button>
        </form>
        ...
        {message && (
          <p
            className={
              message.toLowerCase().includes("failed") ? "error" : "muted"
            }
          >
            {message}
          </p>
            ...
            {videos.map((video) => (
              <div className="video-row" key={video.id}>
                <div>
                  ...
                  <small>{video.channel || "Unknown channel"}</small>

                  <small>{video.ready ? "Ready" : "Processing"}</small>
                </div>
