...
const Scoreboard = forwardRef(function Scoreboard(
  ...
  const sortedPlayers = [...players].sort(
    (a, b) =>
      Number(b?.score ?? b?.points ?? b?.totalScore ?? 0) -
      Number(a?.score ?? a?.points ?? a?.totalScore ?? 0),
  );
  ...
  return (
    <div id="multiScoreboard" ref={ref} className="scoreboard">
      {sortedPlayers.map((player, index) => {
        ...
        const roundScore = roundScores?.get(String(playerId)) ?? 0;

        return (
          <div
            key={String(playerId ?? index)}
